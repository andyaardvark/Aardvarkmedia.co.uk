<?php
/**
 * Template Name: About - Our Modus Operandi
 *
 * @package <theme name>
 * @since 1.0
 */
...
<?php
  ...
  foreach($process as $k=>$p){
    $processGraphHTML.= '<li class="graph-process-'.sanitize_title_with_dashes($p['stage']).'">';
    $processGraphHTML.=   $p['stage'];
    $processGraphHTML.=   $svg[$k];
    $processGraphHTML.= '</li>';

    $detailHTML.='<li>';
    $detailHTML.='<h3 class="sideStripes leadingReset"><span>'.$p['stage'].'</span></h3>';
    $detailHTML.='<img class="about-process-image" alt="" src="" />';
...
<article role="main" class="container no-header">


  <div class="container-grey-odd">

    <!-- HEADING -->
    <hgroup class="headline-borderless gridbase">
      <h1 class="sideStripes"><span><?php echo get_field('heading'); ?></span></h1>
    ...
    <section class="gridbase">
      <ol class="about-process-graph numericColouredList">
  ...
  <div class="container-grey-odd">
    <section class="gridbase">
      <ol class="about-process-desc columnisedList ">
        <?php echo $detailHTML; ?>
      </ol>
    </section>
  </div>
